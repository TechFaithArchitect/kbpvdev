...
import DUE_TODAY_LABEL from "@salesforce/label/c.Chuzo_Frontier_Total_Due_Label";
import DEPOSIT_LABEL from "@salesforce/label/c.Chuzo_Frontier_Required_Deposit_Label";
import BACK_BALANCE_LABEL from "@salesforce/label/c.Chuzo_Frontier_Back_Balance_Label";
import TITLE_LABEL from "@salesforce/label/c.Chuzo_Frontier_Payment_Title";
import AUTOPAY_LABEL from "@salesforce/label/c.Chuzo_Frontier_Autopay_Title";
import REFRESH_LABEL from "@salesforce/label/c.Windstream_Refresh_Fields_credit_check";
import PCI_LABEL from "@salesforce/label/c.Email_PCI_Link_Button_Label";
import PAYMENT_INFORMATION_LABEL from "@salesforce/label/c.Payment_Information_Title";
import PCI_SMS_LABEL from "@salesforce/label/c.SMS_PCI_Link_Button_Label";

export default class Poe_lwcBuyflowFrontierCheckoutTab extends NavigationMixin(LightningElement) {
    @api recordId;
    @api logo;
    @api cart;
    @api origin;
    ...
    labels = {
        ...
        dueToday: DUE_TODAY_LABEL,
        deposit: DEPOSIT_LABEL,
        backBalance: BACK_BALANCE_LABEL,
        title: TITLE_LABEL,
        autoPay: AUTOPAY_LABEL,
        refresh: REFRESH_LABEL,
        pci: PCI_LABEL,
        paymentInformation: PAYMENT_INFORMATION_LABEL,
        pciSms: PCI_SMS_LABEL
    };
    ...
    handleMethod(event) {
        ...
        this.originalValues();
        this.handleAutoPayOptions();
    }

    handleAutoPayOptions() {
        let options = [...this.autoPayDiscounts];
        if (this.isPCI) {
            options = [...options.filter((item) => item.value !== "ach")];
        }
        this.autoPayOptions = [...options];
    }
    ...
    handleCartChange() {
        let hasToday = true;
        let todayCharges = [...this.cartInfo.todayCharges];
        let todayTotal = 0;
        ...
        this.cartInfo = { ...this.cartInfo, todayCharges, hasToday, todayTotal };
    }
                ...
                callEndpoint({ inputMap: myData })
                    .then((response) => {
                        ...
                        if (error) {
                            ...
                            this.backBalanceSumAmount = Number(0.0).toFixed(2);
                            ...
                            if (this.backBalances.length > 0) {
                                ...
                                this.backBalanceSumAmount = Number(this.backBalanceSumAmount).toFixed(2);
                            }
    ...
    logError(errorMessage, request, endpoint = "", type = "Internal Error") {
        const error = {
            ...
            type,
            tab: "Payment",
            component: "poe_lwcBuyflowFrontierCheckoutTab",
            error: errorMessage
        };

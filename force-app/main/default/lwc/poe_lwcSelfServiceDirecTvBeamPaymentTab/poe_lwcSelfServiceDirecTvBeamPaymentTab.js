...
export default class Poe_lwcSelfServiceDirecTvBeamPaymentTab extends NavigationMixin(LightningElement) {
    @api clientInfo;
    @api orderInfo;
    @api recordId;
    @api origin;
    ...
    renderedCallback() {
        if (this.disableAutoPayFields) {
            this.template.querySelectorAll("lightning-input").forEach((input) => {
                if (!input.disabled) return;

                input.reportValidity(); // Remove error messages from disabled autopay inputs
            });

            this.template.querySelectorAll("lightning-select").forEach((input) => {
                if (!input.disabled) return;
    ...
    connectedCallback() {
        ...
        if (this.isGuestUser) {
            ...
            this.agreementLabel = "I agree";
        }

        this.paymentTClabel = `${this.agreementLabel} to the Payment Authorization Terms and Conditions`;
        this.cart = { ...this.cartInfo };
        let header = "";
        if (!this.isGuestUser) {
            header = "<p><strong><em>Please read the following information to the customer:</em></strong></p>";
        }
                ...
                cart.monthlyCharges.forEach((charge) => {
                    if (
                        ...
                        this.handleAutoPayTermsModal(
                            ...
                            () => {
                                this.autoPay = false;
                                this.initialAutoPayValue = false;
                                this.handleAutoPay();
                            }
                        );
                    }
        ...
        if (
            (this.useSameAutopay ||
                (this.autopayFirstName !== undefined &&
                    ...
                    cvvPattern.test(this.autopayCvv) &&
    ...
    handleDeclineAutopayModal() {
        chuzo_modalGeneric
            .open({
                content: {
                    title: "Decline Autopay",
                    provider: "directv",
                    body: this.declineWarning,
                    agreeLabel: "Confirm",
                    canClose: true
                }
            })
    ...
    handleCostumerAutopayChange(event) {
        this.autoPay = event.detail.checked;
        if (this.autoPay) {
            this.handleAutoPayTermsModal(
                () => this.handleAutoPay(),
                () => {
            ...
            this.handleDeclineAutopayModal();
        }
    }

    handleAutoPayTermsModal(agreeFunction, disagreeFunction) {
        chuzo_modalAutoPayTerm
            .open({
                paperlessVerbiage: this.paperlessVerbiage,
                autoPayVerbiage: this.autoPayVerbiage
            })
            .then((result) => {
                this.agreementPaperlessBilling = result?.agreed;
                this.agreementAutoPay = result?.agreed;
    ...
    handleAutoPay() {
        ...
        if (this.autoPay) {
            const hasAutoPayItem = cart.monthlyCharges.some((item) => {
                return item.discount && item.name.includes("AutoPay") && item.name.includes("Paperless Billing");
            });
            if (this.autoPayCharge !== undefined && !hasAutoPayItem) {
    ...
    async handleValidateCard(autopay) {
        ...
        let myData = {
            path,
            ...this.orderInfo,
            cartId: this.cart.orderNumber || this.orderInfo.beam.orderNumber,
    ...
    handleEmail(event) {
        ...
        if (event.target.name == "email") {
            ...
            this.noEmail = this.email !== undefined && emailre.test(this.email) ? false : true;
        } else {

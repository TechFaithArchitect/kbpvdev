...
export default class Poe_lwcBuyflowDirecTvEngaOrderConfirmationTab extends NavigationMixin(LightningElement) {
    @api clientInfo;
    @api orderInfo;
    @api origin;
    @api cartInfo;
                ...
                if (result.hasOwnProperty("error")) {
                    ...
                    switch (result.status.toUpperCase()) {
                        case "COMPLETE":
                            ...
                            this.getOrdersHandler(result.value.createOrderResponse.data.orderIdentifier.omsOrderId);
                            break;
                        case "FAILURE":
                            console.error(error, "ERROR");

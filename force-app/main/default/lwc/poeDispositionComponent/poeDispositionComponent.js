...
import ZIP_CODE_FIELD from "@salesforce/schema/Opportunity.Maps_PostalCode__c";
import StageName from "@salesforce/schema/Opportunity.StageName";
import Id from "@salesforce/user/Id";
import OPP_ORIGIN from "@salesforce/schema/Opportunity.POE_Clicker_Origin__c";
...
export default class poeDispositionComponent extends NavigationMixin(LightningElement) {
    ...
    get iconButtonClass() {
        return this.hasOutcome ? "chuzoFooterStyleDisabled" : "chuzoFooterStyle";
    }
    ...
    userDetails({ error, data }) {
        if (data) {
            ...
            this.isDoorToDoor = repType.includes("Door To Door");
            this.checkFlag();
        } else if (error) {
            this.error = error;
    ...
    @wire(getRecord, {
        recordId: "$recordId",
        fields: [FRAUD_FIELD, OPP_ORIGIN, POE_Outcome__c, StageName, ZIP_CODE_FIELD]
    })
    wiredRecord({ error, data }) {
        if (error) {
            console.error("wiredRecord ERROR", error);
        } else if (data) {
            if (this.negativeOutcomes.indexOf(data.fields.POE_Outcome__c.value) != -1) {
                this.hasOutcome = true;
            }
            this.isOppFlagged = data.fields.POE_FraudObservation__c.value;
            this.opportunityOrigin =
    ...
    orderHandler(event) {
        ...
        if (this.savedOutcome === undefined || this.savedOutcome === null) {
            this.loaderSpinner = true;
            this.createGeoLocationRecord(true);
        } else {
            switch (this.opportunityOrigin) {
                case "phonesales":
                    this[NavigationMixin.Navigate]({
                        type: "comm__namedPage",
                        attributes: {
    ...
    outcomeHandler(event) {
        this.selectedOutcome = event.target.dataset.id;
    }

    handleSubmit(event) {
        event.preventDefault();
        this.createGeoLocationRecord(false);
                        ...
                        if (redirect) {
                            ...
                            this[NavigationMixin.Navigate]({
                                ...
                                state: {
                                    ...
                                    clickerorigin: this.opportunityOrigin
                                }
                            });
                        }
                    })
